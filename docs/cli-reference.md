...
After `enable --now`, setup runs a 20-second **active-state verification**: `systemctl is-active ccrouter` must report `active` continuously for 3s, with no `NRestarts` increment during that hold. A single restart-and-recover within the 20s window is tolerated — so a service whose first ExecStart crashes (e.g. freshly-created `ccrouter` system user's group membership not yet visible to systemd's first fork) and then comes back via `Restart=always` is considered healthy. Two or more `NRestarts` increments, or failure to sustain `active`, surfaces the last 30 journalctl lines + `systemctl status` and exits non-zero — no false-positive "Service is active" if it's actually crash-looping. Common failure diagnostics appear inline in the error message.
